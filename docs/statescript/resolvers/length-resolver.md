...
Resolves the length (magnitude) of a vector operand. Returns a `float`. Supports `Vector2`, `Vector3`, and `Vector4`. Scalar, quaternion, and plane types are not supported.
...
- Calls `.Length()` on the resolved vector value.

...
public class GameplayTagContainerTests(GameplayTagsManagerFixture fixture) : IClassFixture<GameplayTagsManagerFixture>
{
	...
	{
		GameplayTagContainer container = ConstructContainer(tagKeys);
	...
	public void Container_deserializes_successfully(byte[] netIndexStream, string[] tagKeys)
	{
		...
		GameplayTagContainer tagContainerCheck = ConstructContainer(tagKeys);
	...
	[InlineData(
		new string[] { "item.consumable.potion.mana", "color.blue" },
		"enemy")]
	public void Container_adds_tag_sucessfully(string[] tagKeys, string tagKey)
	{
		GameplayTagContainer tagContainer = ConstructContainer(tagKeys);

		var tag = GameplayTag.RequestTag(_gameplayTagsManager, tagKey);

		tagContainer.AddTag(tag);

		tagContainer.Should().Contain(tag);
	}
	...
	public void Container_does_not_add_Empty_tag(string[] tagKeys)
	{
		...
		tagContainer.Should().NotContain(GameplayTag.Empty);
		tagContainer.Count.Should().Be(tagKeys.Length);
	}

	[Theory]
	[Trait("Remove", "Remove tag")]
	[InlineData(
		new string[] { "tag" },
		"tag")]
	[InlineData(
	...
	public void Container_does_not_contain_tag_removed(string[] tagKeys, string tagKey)
	{
		...
		tagContainer.Should().NotContain(tag);
		tagContainer.Count.Should().Be(tagKeys.Length - 1);
	}

	[Theory]
	[Trait("Remove", "Remove tags")]
	[InlineData(
		new string[] { "tag" },
		new string[] { "tag" })]
	[InlineData(
	...
	{
		GameplayTagContainer tagContainer = ConstructContainer(containerTagKeys);
		GameplayTagContainer removeTagsContainer = ConstructContainer(tagsToRemove);

		tagContainer.RemoveTags(removeTagsContainer);

		tagContainer.HasAnyExact(removeTagsContainer).Should().BeFalse();
		tagContainer.Count.Should().Be(containerTagKeys.Length - tagsToRemove.Length);
	}
	...
	{
		GameplayTagContainer tagContainer = ConstructContainer(["tag", "simple.tag", "other.tag"]);
	...
	{
		GameplayTagContainer tagContainerA = ConstructContainer(tagKeysA);
		GameplayTagContainer tagContainerB = ConstructContainer(tagKeysB);
		...
		string[] tagKeysResult)
	{
		...
		GameplayTagContainer validationContainer = ConstructContainer(tagKeysResult);
	...
	public void Container_A_append_container_B_has_all_tags_from_both(string[] tagKeysA, string[] tagKeysB)
	{
		...
		tagContainerA.HasAllExact(tagContainerA).Should().BeTrue();
		tagContainerA.HasAllExact(tagContainerB).Should().BeTrue();
	}
	...
	public void Container_append_empty_container_has_all_container_tags()
	{
		...
		tagContainer.HasAllExact(tagContainer).Should().BeTrue();
	}
	...
	public void Empty_container_append_container_has_all_container_tags()
	{
		...
		emptyContainer.HasAllExact(tagContainer).Should().BeTrue();
	}

	[Fact]
	[Trait("Append", "Empty container")]
	public void Empty_container_append_empty_container_continues_empty()
	{
		...
		emptyContainerA.AppendTags(emptyContainerB);

		emptyContainerA.Should().BeEmpty();
		emptyContainerB.Should().BeEmpty();
	}
	...
	private GameplayTagContainer ConstructContainer(string[] tagKeys)
	{
		var tagSet = new HashSet<GameplayTag>();
		...
		return new GameplayTagContainer(_gameplayTagsManager, tagSet);
	}
